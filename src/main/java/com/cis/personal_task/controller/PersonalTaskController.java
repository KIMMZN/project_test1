...
//        model.addAttribute("task_status", status);
//        return "task_list";
//    }

    // 업무 상태 완료 처리
    @PostMapping("/{task_num}/complete")
    public String completeTask(@PathVariable int task_num) {
        personalTaskService.updateTaskStatus(task_num, "완료");
        return "redirect:/tasks?status=진행&page=1";
    }

    // 파일 업로드
    @PostMapping("/{task_num}/upload")
    public ResponseEntity<String> uploadFiles(@PathVariable int task_num, @RequestParam("files") List<MultipartFile> files) {
        if (files.size() > 3) {
...
//    }
    ...
    // 업무 상태 업데이트 API
    @PatchMapping("/{task_num}/task_status")
    public ResponseEntity<String> updateTaskStatus(@PathVariable int task_num, @RequestParam String task_status) {
        if (!"진행".equals(task_status) && !"완료".equals(task_status)) {
